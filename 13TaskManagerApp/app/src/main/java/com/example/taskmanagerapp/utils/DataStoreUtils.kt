...
object DataStoreUtils {
    ...
    fun readTasks(context: Context): Flow<String> = context.dataStore.data.map { pref ->
        pref[TASKS_KEY] ?: ""
    }
    suspend fun saveTheme(context: Context, isDark: Boolean) {
        context.dataStore.edit { pref ->
            pref[THEME_KEY] = isDark
        }
    }
    fun readTheme(context: Context): Flow<Boolean> = context.dataStore.data.map { pref ->
